...
## format
By default, parsed XML is a single line XML string. By `format: true`, you can format it for a better view.

## ignoreAttributes
Don't consider attributes while building XML. Other attribute-related properties should be set to correctly identify an attribute property.

## indentBy
Applicable only if `format:true` is set.

## preserveOrder
When you parse an XML using XMLParser with `preserveOrder: true`, the result JS object has a different structure. So parse that structure in the original XML, you should set the same option while building the XML from that js object.
...
This property groups all the children tags in single parent tag.
...
To suppress an unpared tag from `<br/>` to `<br>`.
...
## unpairedTags
Unpaired Tags are the tags that don't have matching closing tag. Eg `<br>` in HTML. You can parse unpaired tags by providing their list to the parser, validator and builder.
...
Now if you set `suppressUnpairedNode: false`. You'll get the following output
...
When you use XML Parser with `alwaysCreateTextNode: true`, it doesn't impact the XMLBuilder result
...
When you use XML Parser with `isArray`, it doesn't impact XMLBuilder results under some extent.
